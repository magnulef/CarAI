        ...
        return InputContract.builder()
            .withVisionEdges(
                ...
                VisionContract.builder()
                    ...
                    .withFrontAdd20(vision(angle, 0.20, start, 200, RewardGates.getLines()))
                    .withFrontMinus20(vision(angle, -0.20, start, 200, RewardGates.getLines()))
                    .withFrontAdd40(vision(angle, 0.40, start, 200, RewardGates.getLines()))
                    .withFrontMinus40(vision(angle, -0.40, start, 200, RewardGates.getLines()))
                    .withFrontAdd60(vision(angle, 0.60, start, 200, RewardGates.getLines()))
                    .withFrontMinus60(vision(angle, -0.60, start, 200, RewardGates.getLines()))
                    .withFrontAdd80(vision(angle, 0.90, start, 200, RewardGates.getLines()))
                    .withFrontMinus80(vision(angle, -0.90, start, 200, RewardGates.getLines()))
                    .withFrontAdd100(vision(angle, 1.0, start, 200, RewardGates.getLines()))
                    .withFrontMinus100(vision(angle, -1.0, start, 200, RewardGates.getLines()))
                    .withFrontAdd120(vision(angle, 1.20, start, 200, RewardGates.getLines()))
                    .withFrontMinus120(vision(angle, -1.20, start, 200, RewardGates.getLines()))
                    .withFrontAdd140(vision(angle, 1.40, start, 200, RewardGates.getLines()))
                    .withFrontMinus140(vision(angle, -1.40, start, 200, RewardGates.getLines()))
                    .withFrontAdd160(vision(angle, 1.60, start, 200, RewardGates.getLines()))
                    .withFrontMinus160(vision(angle, -1.60, start, 200, RewardGates.getLines()))
                    .build()
            ).withDirectionX(
                this.direction.x
            ).withDirectionY(
